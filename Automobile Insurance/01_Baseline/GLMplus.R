...
{t1 <- proc.time()
d.glm <- glm(FraudFound ~ daysDiff + Deductible + Age + log(Age) + I(Age ^2) + I(Age ^3) +
               I(Age ^4) + Fault + PastNumberOfClaims + VehiclePrice + AddressChange.Claim +
               Make + DriverRating + VehicleCategory + NumberOfSuppliments + MaritalStatus +
               BasePolicy + AccidentArea + PoliceReportFiled + AccidentArea * PoliceReportFiled +
               Age*Make + DriverRating*MaritalStatus + VehiclePrice*Make + Fault*Age,
